...
import json, requests, re, phonenumbers, logging
import csv
...
def fetch_contacts(link,email_patterns):
    ...
    if valid_emails or valid_phones:
        ...
        print("== CONTACT FOUND == ", contact)  # Debugging line
        return contact
    return None
...
def index():
    if request.method == "POST":
        ...
        email_patterns = [e.strip() for e in request.form["email_patterns"].split(",")]  
        site_domain = [d.strip() for d in request.form["site_domain"].split(",")]
        ...
        country_code = request.form.get("country", "IN")  # default: IN (India)
        ...
        for keyword in industries:
            ...
            while start < 100:
                search = GoogleSearch({
                    ...
                    "safe": "active",
                    "engine": "google",
                    #"ll": "@26.8467,80.9462",  # Lucknow coordinates
                    "api_key": api_key
                })
